    ...
    public class ExtensionConfig
    {
        ...
        public void WriteDLaBModelBuilderProperties(Utf8JsonWriter writer, EarlyBoundGeneratorConfig settings)
        {
            ...
            if (!string.IsNullOrWhiteSpace(settings.SettingsFilePath))
            {
                ...
                var settingsBase = Path.GetFileNameWithoutExtension(settings.SettingsFilePath);
                writer.AddProperty("SerializedMetadataRelativeFilePath", Path.Combine(settingsDir, settingsBase + ".metadata.xml"));
            }
